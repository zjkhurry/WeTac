...
public class Demo : MonoBehaviour
{
    ...
    public void SelectCharacteristic(GameObject data)
    {
        ...
        print(selectedCharacteristicId);
        subscribeButton.interactable = true;
        writeButton.interactable = true;
    }
    public void SelectCharacteristic_Initialization()
    {
        selectedCharacteristicId = "0000fff2-0000-1000-8000-00805f9b34fb"; //Frequency
        subscribeButton.interactable = true;
        writeButton.interactable = true;
        byte Tenp1 = 25;
        byte[] Tenp1_array = { 0 };
        Tenp1_array[0] = Tenp1;
        WriteCallFunction(Tenp1_array);
        print("Frequency_write");

        selectedCharacteristicId = "0000fff3-0000-1000-8000-00805f9b34fb"; //Duty
        subscribeButton.interactable = true;
        writeButton.interactable = true;
        Tenp1 = 5;
        Tenp1_array[0] = Tenp1;
        ...
        selectedCharacteristicId = "0000fff4-0000-1000-8000-00805f9b34fb"; //current
        subscribeButton.interactable = true;
        writeButton.interactable = true;
        Tenp1 = 70;
        Tenp1_array[0] = Tenp1;
    ...
    public void WriteCallFunction(byte[] inputWrite) //This function need to call by other script
    {
        byte[] payload = inputWrite;
        BleApi.BLEData data = new BleApi.BLEData();
        data.buf = new byte[512];
        data.size = (short)payload.Length;
        data.deviceId = selectedDeviceId;
        data.serviceUuid = selectedServiceId;
        data.characteristicUuid = selectedCharacteristicId; //UUID for channel write
        for (int i = 0; i < payload.Length; i++)
            data.buf[i] = payload[i];
        // no error code available in non-blocking mode
        BleApi.SendData(in data, false);
        print("Normal Send write");
    }

    public void WriteCallFunction2(byte[] inputWrite) //This function need to call by other script
    {
        byte[] payload = inputWrite;
        BleApi.BLEData data = new BleApi.BLEData();
        data.buf = new byte[512];
        ...
        data.deviceId = selectedDeviceId;
        data.serviceUuid = selectedServiceId;
        data.characteristicUuid = "0000fff1-0000-1000-8000-00805f9b34fb"; //UUID for channel write
        for (int i = 0; i < payload.Length; i++)
            data.buf[i] = payload[i];
        // no error code available in non-blocking mode
        BleApi.SendData(in data, false);
    ...
    public void PrintSthn() //This function need to call by other script
    {
        print("Is working");
        byte[] lol = Encoding.UTF8.GetBytes("2");
        byte lol2 = 2;
        //print(lol2.Length);
        print(lol2);
        //print(Encoding.UTF8.GetBytes("1"));
    }
}
